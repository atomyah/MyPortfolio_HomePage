...
const Register = () => {
    ...
    const onSubmit: SubmitHandler<RegisterForm> = async (data) => {
        const response = await fetch("http://localhost:3000/api/user/register", { // api/user/registerではダメだった…
        method: "POST",
        headers: {
      ...
      <Container maxWidth="md">
        <Box mb={6} mt={6}>
          <Typography align="center" fontSize={18}>
            ...
            <br />
        {registerSuccess && <p style={{ color: 'mediumvioletred', fontSize: '16px' }}>ユーザー登録が完了しました</p>}
        </Box>
        <Box>
